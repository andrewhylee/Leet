 ...
 * @param {*} nums
 * @returns
 * https://leetcode.com/problems/permutations/solutions/2542099/javascript-dfs-backtracking-with-heavy-comments/
 */
const permute2 = (nums) => {
  // Backtracking
...
var permute = function (nums) {
  ...
  const backtracking = (remainingNums) => {
    if (remainingNums.length === 0) {
      result.push([...tempDataStruct]);
      return;
    }
    for (const i in remainingNums) {
      tempDataStruct.push(remainingNums[i]);
      let newRemainingNums = [...remainingNums];
      newRemainingNums.splice(parseInt(i), 1);
      backtracking(newRemainingNums);
      tempDataStruct.pop();
    }
  };
